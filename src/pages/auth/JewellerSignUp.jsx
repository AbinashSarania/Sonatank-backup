...
import { signUpJeweller } from "../../api/authAPI"; // Import the signUpJeweller function
import signupImage from "../../assets/images/auth-images (4).jpeg"; // Adjust the path to your image
import { Link, useNavigate } from "react-router-dom"; // Import Link and useNavigate
import { FaArrowLeft } from "react-icons/fa"; // Import the left arrow icon

const JewellerSignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Call the API function to sign up a jeweller
      const response = await signUpJeweller(email, password);
      setSuccessMessage("Jeweller created successfully!");
      setErrorMessage(""); // Clear any previous errors
      setEmail("");
      setPassword("");
    } catch (error) {
      setErrorMessage(error.error || "Failed to create jeweller");
